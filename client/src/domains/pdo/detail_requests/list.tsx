        ...
        scroll={
          <Stack sx={{ p: 1 }}>
            ...
            <Table
              columns={columns}
              data={store.requests}
              onRowClick={request => store.setSelectedId(request.id)}
            />
          </Stack>
        }
      />
      <DetailRequestDetailsModal store={store} />
    </>
  )
})

const DetailRequestDetailsModal = observer(
    ...
    return (
      <>
        <InModal
          ...
          setOpen={open => !open && store.closeDetails()}
        >
          {!request ? (
            <Loading />
          ) : (
