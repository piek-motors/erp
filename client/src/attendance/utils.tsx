...
import { Employee, PreparedEmployeeDto } from 'lib/types/global'
...
export function queryVariables(selectedMonth: SelectedMonth) {
  ...
  const lte =
    year + '-' + m + '-' + moment({ year, month }).daysInMonth() + 'T23:59:59'
  return { gte, lte }
}

export function prepareEmployeeData(
  employee: Employee,
  state: State
): PreparedEmployeeDto {
  ...
  const intervals = ArrayFromDaysInMonth.createOfType<
    PreparedEmployeeDto['intervals'][number]
  >(state.selectedMonth, {}).map<TPreparedInterval>((_, index) => {
    const day = index + 1
    // интервалы сотрудника в течении дня
    const usrIntervals = employee.intervals.filter(
      each => new Date(each.ent).getDate() === day
    )
  ...
  const { monthlyWithRetention, monthlyWithRetentionResolved } =
    intervals.reduce<{
        ...
        return cur.resolved
          ? {
              ...
              monthlyWithRetentionResolved:
                acc.monthlyWithRetentionResolved + dur
            }
          : {
              monthlyWithRetentionResolved:
                acc.monthlyWithRetentionResolved + dur,
