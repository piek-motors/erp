...
import { Inp, LoadingHint, observer, P, UseIcon, useState } from 'lib/index'
...
export const UpdateGroupNameModal = observer(() => {
  const handleSubmit = async (name: string) => {
    ...
    await store.updateGroup(store.targetGroup.group.id, name)
  }
  ...
  return (
    <BaseGroupModal
      openButton={
        ...
        >
          {store.targetGroup?.group.name}
        </P>
      }
      title="Изменить название группы"
      onSubmit={handleSubmit}
      initialName={store.targetGroup?.group.name || ''}
    />
  )
})
